...
class OllamaManager:
    ...
    def select_best_model(self, task_description: str, options: List[str] = None) -> str:
        ...
        This is a simple implementation that could be enhanced with more sophisticated logic.
        ...
        if options is None or len(options) == 0:
            # Default models based on task complexity
            if any(keyword in task_description.lower() for keyword in
                   ["complex", "difficult", "advanced", "reasoning"]):
                # Complex task - use a more capable model if available
                if "sqlcoder:15b" in self.get_all_available_models():
                    self._logger.info("Selected model for complex task: sqlcoder:15b")
                    return "sqlcoder:15b"
                else:
                    self._logger.info(f"Selected default model: {self.default_model}")
                    return self.default_model
            else:
                # Standard task - use specialized model based on description
                if "sql" in task_description.lower() and "sqlcoder:15b" in self.get_all_available_models():
                    self._logger.info("Selected model for SQL task: sqlcoder:15b")
                    return "sqlcoder:15b"
                elif any(keyword in task_description.lower() for keyword in ["code", "programming", "syntax"]) and "opencoder:8b" in self.get_all_available_models():
                    self._logger.info("Selected model for coding task: opencoder:8b")
                    return "opencoder:8b"
                else:
                    self._logger.info(f"Selected default model: {self.default_model}")
                    return self.default_model
            ...
            if "sqlcoder:15b" in options and "sql" in task_description.lower():
                self._logger.info("Selected model from options: sqlcoder:15b")
                return "sqlcoder:15b"
            elif "opencoder:8b" in options and any(keyword in task_description.lower() for keyword in ["code", "programming", "syntax"]):
                self._logger.info("Selected model from options: opencoder:8b")
                return "opencoder:8b"
            else:
...
class EnhancedOllamaLLM(LLM):
    ...
    def __init__(self, **kwargs):
        ...
        # Initialize the CrewAI-compatible OllamaLLM
